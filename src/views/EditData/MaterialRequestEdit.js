...
const [arabic, setArabic] = useState([]);
  ...
  const editTenderById = () => {
    api.post('/materialrequest/getMaterialRequestById', { material_request_id: id }).then((res) => {
      setTenderDetails(res.data.data[0]);
      getContact(res.data.data.company_id);
    });
  };

  const [status, setStatus] = useState(tenderDetails && tenderDetails.material_status);
  ...
  const editTenderData = () => {
    ...
    api
      .post('/materialrequest/editMaterialRequest', tenderDetails)
      .then(() => {
  ...
  // Get Line Item
  const getLineItem = () => {
    api.post('/materialRequest/getMRLineItemsById', { material_request_id: id }).then((res) => {
  ...
  const deleteRecord = (deleteID) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        api.post('/materialrequest/deleteMaterialRequestItem', { material_request_item_id: deleteID }).then(() => {
                          ...
                          <tr key={e.material_request_id}>
                            ...
                            <td data-label="Actions">
                              ...
                              <span
                                className="addline"
                                onClick={() => {
                                  deleteRecord(e.material_request_item_id);
                                }}
                              >
                                <Icon.Trash2 />
                              </span>
