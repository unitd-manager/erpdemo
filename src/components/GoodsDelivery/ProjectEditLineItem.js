...
const InvoiceModal = ({ editModal, setEditModal, getgoodsLineItemById,id }) => {
  ...
  const getLineItem = () => {
    api.post('/projectgoodsdelivery/getgoodsdeliveryitemById', { project_goods_delivery_id: id }).then((res) => {
  ...
  const editLineItemApi = () => {
    const editPromises = addLineItem.map((item) => {
      return api.post('/projectgoodsdelivery/edit-goodsdeliveryitem', item);
    });
