    ...
    api.delete(`/finance/deleteorder_item/${quoteId}`).then(() => {
      api
        ...
        .then((res) => {
          ...
          api
            .get('/projectsalesorder/checkOrderItems')
            .then((response) => {
              ...
              const insertOrderItems = (index) => {
                if (index < quoteItems.length) {
                  ...
                  if (ExistingOrderItemsId.includes(QuoteItem.project_quote_id)) {
                    console.warn(
                      `Project Order item for quote id  ${QuoteItem.project_quote_id} already exists, skipping insertion`,
                    );
